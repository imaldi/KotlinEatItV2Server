...
import com.aim2u.kotlineatitv2server.adapter.MyAddonAdapter
import com.aim2u.kotlineatitv2server.adapter.MySizeAdapter
import com.aim2u.kotlineatitv2server.common.Common
import com.aim2u.kotlineatitv2server.eventbus.*
...
class SizeAddonEditActivity : AppCompatActivity() {
    ...
    fun onSizeModelUpdate(event:UpdateSizeModel){
        if(event.sizeModelList != null){
            if (Common.foodSelected?.size != null)//size
            {
                needSave = true
